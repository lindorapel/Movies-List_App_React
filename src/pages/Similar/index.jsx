...
const Similar = () => {
  ...
  const fetchSimilarMovies = async (page) => {
    try {
  ...
  const loadNextPage = async () => {
    ...
    try {
      ...
      const data = await fetchSimilarMovies(nextPage);

      setSimilarMovies((prevMovies) => [...prevMovies, ...data]);
  ...
  useEffect(() => {
    const getSimilarMovies = async () => {
      try {
        const data = await fetchSimilarMovies(1);
